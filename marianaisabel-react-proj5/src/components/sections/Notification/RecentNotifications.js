...
function RecentNotifications({userPriv, ...props}) {
    ...
    useEffect(async()=>{
        ...
        const getLastNotification = await get("notifications/lastNotifications")
        if (response.ok) {
            console.log(getLastNotification)
            setLastNotification(getLastNotification);
            if(userPriv =="ADMIN"){
                setAdminPriv(true);
            }
            setAppError("");
  ...
  return (lastNotification && lastNotification.length?
    ...
    > 
    <Heading as='h3' size='lg' color="black"><FormattedMessage id={"last_notifications"} /> </Heading>
    {error && 'Error!'}
    {loading && 'Loading...'}
